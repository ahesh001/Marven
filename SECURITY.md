...
- Retrieval-run capture is disabled by default. Plaintext capture retains user queries for evaluation; hash-only capture avoids retaining query text but still stores scoped IDs and ranking metadata. Treat both forms, their labels, and exports as sensitive local data and delete them when no longer needed.
- The retrieval-label endpoints are unauthenticated prototype-local interfaces. Do not expose them to an untrusted network or accept client-supplied owner scope in a multi-user deployment.
- The optional Graphiti harness is a one-way offline experiment, not a runtime trust boundary. It refuses remote model or graph endpoints by default, disables Graphiti telemetry and raw episode storage, and has no canonical write-back path. Its separate graph database still contains derived entities and facts and must be protected and disposed of according to the same consent and deletion policy as source memory.
